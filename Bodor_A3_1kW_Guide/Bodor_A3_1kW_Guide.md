...
1. **Set Origin** ![](./Button/SetOrigin.png) - установить ноль *рабочих* координат. Ноль для всех деталей переместится в абсолютные (текущие) координаты головы. После нажатия этой кнопки и перемещения деталей на экране, изображение деталей на экране будет отображать то, что произойдет, если запустить программу на выполнение.
...
Фактически если переместить голову в другое место и запустить программу на выполнение, то будет автоматически выполнена команда **Set Origin**.

# Запуск программы.

**ИЗОБРАЖЕНИЕ НА ЭКРАНЕ ОБМАНЧИВО!!!**

**ТЕКУЩЕЕ ПОЛОЖЕНИЕ ГОЛОВЫ ВСЕГДА ЯВЛЯЕТСЯ НАЧАЛЬНЫМИ КООРДИНАТАМИ ДЛЯ ПРОГРАММЫ РЕЗКИ!!!**

**ДЕТАЛЬ ПОКАЗАНА В НАЧАЛЕ КООРДИНАТ РАБОЧЕГО СТОЛА, НО ФАКТИЧЕСКИ ОНА БУДЕТ ВЫРЕЗАТЬСЯ С ТЕКУЩИХ (АБСОЛЮТНЫХ) КООРДИНАТ ГОЛОВЫ!!!**
...
3. Кнопка **Frame** ![](./Button/Frame.png) заставит голову станка описать в воздухе необходимые габариты листа (и его положение), которые нужны для выполнения текущей программы (проверка, влезут ли детали на лист метала, который лежит на столе).
4. Кнопка **Trace** ![](./Button/Trace.png) запустит передвижение головы без включения лазера (для проверки программы).
5. Кнопка **Simu** ![](./Button/Simu.png) запустит симуляцию программы на ПО, голова не двигается, лазер не включается.
6. Кнопка **▶** запустит программу резки.
7. После окончания, можно вручную разрезать металл на остатки кнопкой **Cut Sheet** ![](./Button/CutSheet.png).
8. Кнопка **Select** ![](./Button/Selected.png) позволяет резать только выделенный контур. Если кнопка нажата, но не выделен ни один контур, появится ошибка (перевод ужасен) при запуске программы. Выделяем контуры, нажимаем кнопку (она остается нажатой) и запускаем программу на выполнение.
...
*Cut Direct*: X:negative to positive - первый рез пойдет от нуля в негативную сторону, второй рез пойдет от нуля в позитивную сторону.
