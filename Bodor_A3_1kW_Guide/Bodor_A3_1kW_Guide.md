...
13. Установить голову станка в ноль по **X**,**Y**,**Z** кнопка **Go Home** ![](./Button/GoHome.png).
...
15. Калибровка емкостного датчика высоты **Z** головы над поверхностью:
    1. С пульта, **калибровка одной кнопкой**, быстрее но не видно результатов калибровки на экране (в системных настройках есть опция авто-отображения результатов после окончания калибровки, она работает и для быстрой калибровки и для кнопки **Calibrate** ![](./Button/Calibrate.png), но возможно потребуется перезапуск ПО, чтобы она вступила в силу).
    2. В ПО:
...
13. **System Parameters -> 4.0.0 General -> Automatically switch to the calibration data interface during calibration -> Yes** работает и для кнопки ![](./Button/Calibrate.png) и **One Key Calibrate** при запуске с пульта, но требует перезапуска ПО, для полноценной работы.
...
1. Закрыть баллоны с газом, выкрутить редукторы (понизить давление на выходе, вывести пружину из работы).
...
*Пробитие* выполняется при отрицательном фокусе, концентрируя максимум мощности лазерного луча в толще металла для быстрейшего прокола.

![](./Image/LayerSettingsBigGraph.png)

**Layer -> Big Graph -> Special Technic -> Pre-piercing** позволяет немного ускорить резку (не нужно будет перестраивать фокус при переходе от *пробивки* к резке), сделав всю пробивку за один раз, сразу после старта и только после этого начав резку.

![](./Image/LayerSettingsGlobalParams.png)

**Layer -> Global Params -> Other Param -> Auto group prepunch** возможно позволяет группировать *пробития* в группы и прокалывать не все пробития за раз (при включенном **Pre-piercing**) а только внутри группы, после вырезать детали внутри этой группы, далее проколоть за раз другую группу и после вырезать детали и так далее...
...
**Layer -> Big Graph -> Special Technic -> Micro Joint at High Speed** позволяет делать *перемычку* без остановки на *пробитие* (если не задана *врезка* для *перемычки*), но это приводит к очень большим брызгам. Если ее отключить, то голова остановится для *пробития* и для охлаждения на время **Cool Delay**.
