...
11. Если после размещения деталей на листе нужно что-то исправить в одном типе детали, можно ее выделить, нажать **Edit -> Quantity Modify...** (**Shift + E**), внести изменения и подтвердить их кнопкой **Ok**. Изменения применятся для всех таких же деталей на листе.
...
2. Зажать **Ctrl** чтобы повернуть деталь, **Alt** чтобы растянуть или сжать деталь.
3. Автоматически задать разное направление реза (*CCW* for *Fill* and *CW* for *Unfill*) для выделенных деталей можно только в **One-Click Setting** ![](./Icon/OneClickSetting.png). В **Reverse** ![](./Icon/Reverse.png) сделать этого нельзя, действие не применяется к выделенным деталям!
4. **Nest -> One Key Nesting...** позволяет разместить детали разных видов, (находящиеся в *Draft*), на листе заданного размера.
5. **Nest -> Bestre Nesting...** позволяет замостить лист заданного размера деталью одного вида (находящейся в *Draft*).
6. Дополнительные иконки с настройками, которые больше нигде не появляются (sic!):
   1. Создать *Nest*.
...
12. ![](./Button/Fan.png) вручную включить вентилятор вытяжки (в настройках задано автоматическое включение при начале резки).
...
15. **BodorThinker3.0** это ребрендинг оригинального **NcEditor** от **Weihong**.
16. ![](./Button/LowerIn.png) ![](./Button/UpperIn.png) для профессиональных моделей станков (model P), в которых есть два подвижных стола, один над другим. Нажатие на кнопку указывает, какой стол будет рабочим (один стол уезжает в заднюю часть станка, а другой приезжает в переднюю часть станка под голову лазера).
17. Перед применением **Kerf Compensation** текст должен быть преобразован в графику (**Optimize -> Convert Text to Graphic**).
18. При обработке металла, содержащего цинк (покрытие или латунь), могут выделяться пары цинка!
19. Настройки пробития можно почитать в **NcEditor-v12-User-Manual.pdf** (раздел 2.2.1.2).
20. Подробное описание калибровки и слежения за высотой головы можно почитать в **NcEditor-v12-User-Manual.pdf** (раздел 2.3).
21. Подробное описание процесса **Go Home** можно почитать в **NcEditor-v12-User-Manual.pdf** (раздел 3.4.3).
22. Подробное описание коррекции ошибок позиционирования можно почитать в **NcEditor-v12-User-Manual.pdf** (раздел 3.6).
23. **Mitsubishi-Fiber-Optic-Laser-Cutting-Guide-Ru.pdf & Mitsubishi-CO2-Laser-Cutting-Guide-Ru.pdf** описание некачественной кромки с картинками и советами по решению проблемы.
24. Прежде чем что-то менять в настройках нужно проверить защитное стекло и сопло на предмет загрязнения и повреждения, проверить центровку.
25. При резке рядом с кромкой материала, где меняется электростатическая емкость последнего, высока вероятность контакта между кончиком сопла и заготовкой. Убедитесь в том, что расстояние между вырезаемой деталью и краем листа составляет не менее толщины последнего.
26. Так же, как и на кромке материала, электростатическая емкость меняется, если зазор между деталями мал. Рекомендуется, чтобы зазор между деталями составлял не менее толщины листа (в зависимости от формы детали из этого правила могут быть сделаны исключения).
...
**Edge Seek** - определение положения листа на столе (нету).
